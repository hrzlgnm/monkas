...
monkaS::monkaS()
    ...
    , maportid{mnl_socket_get_portid(manl.get())}
    , maseq{}
{
...
void monkaS::yoink()
{
    ...
    while (ret > 0) {
        ...
        ret = mnl_cb_run(&mabuf[0], ret,
                maca == monka_state::listen_for_changes ? 0 :  maseq,
                maportid, &monkaS::mnl_data_callback, this);
        if (ret < MNL_CB_STOP) {
            printf("bork\n");
            break;
        }
...
void monkaS::ensure_interface_is_named(int interface_index)
{
    ...
    if (matb[IFLA_IFNAME]) {
        ...
        if (name) {
            ...
            std::cerr << "failed to determine interface name from index " << interface_index << "\n";
        }
    }
}
...
{
    if (maca != monka_state::listen_for_changes) return;
    std::cout << "state: \n";
...
monka_steer::monka_steer()
    : maoperstate{}
{

}

const std::string &monka_steer::name() const
{
    return maname;
}
...
uint16_t monka_steer::operstate() const
{
    return maoperstate;
}
